...
class WC_Local_Pickup_Plus_Public_Holidays {
	 ...
	 * @since 2.3.5
	 *
	 * @param string|int $date timestamp or date in PHP format
	 * @return bool
	 */
	...
	/**
	 * Returns unavailable dates according to the public holidays calendar between a range of dates.
	 *
	 * TODO remove this deprecated method by version 2.6 {FN 2018-01-08}
	 *
	 * @since 2.0.0
	 ...
	 * @param int|string $start_date date to calculate unavailable dates from
	 * @param int|string $end_date date to calculate unavailable dates to
	 * @param int $available_days optional total number of days that should be guaranteed to be free of holidays at the end of the range
	 * @return string[]|int[] array of dates in the specified $format
	 */
	public function get_unavailable_dates( $format = 'mysql', $start_date, $end_date, $available_days = 0 ) {
		_deprecated_function( 'WC_Local_Pickup_Plus_Public_Holidays::get_unavailable_dates()', '2.3.5' );
		return array();
	}
