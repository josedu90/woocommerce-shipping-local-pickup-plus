 ...
 * @since 2.0.0
 */
class WC_Local_Pickup_Plus_Lifecycle {


	/**
	 * Run install scripts.
	 *
	 * @see \WC_Local_Pickup_Plus::install()
	 *
	 * @since 2.0.0
	 */
	...
	/**
	 * Get table names used by Local Pickup Plus.
	 *
	 * @since 2.0.0
	 *
	 * @return string[]
...
CREATE TABLE {$wpdb->prefix}woocommerce_pickup_locations_geodata (
  ...
  KEY country_state (country ASC, state ASC)
	 ...
	 * Run upgrade scripts.
	 *
	 * @see \WC_Local_Pickup_Plus::upgrade()
	 *
	 * @since 2.0.0
	 *
	 * @param string $from_version the version that is being updated
	 */
	public static function upgrade( $from_version ) {

		// versions before 1.13.5 did not have a version flag
		if ( empty( $from_version ) || version_compare( $from_version, '1.13.5', '<' ) ) {
			$from_version = '1.13.4';
		}

		$update_path = array(
			'1.13.5' => 'update_to_1_13_5',
			'2.0.0'  => 'update_to_2_0_0',
			'2.3.4'  => 'update_to_2_3_4',
		);

		foreach ( $update_path as $update_to_version => $update_script ) {
			if ( version_compare ( $from_version, $update_to_version, '<' ) && method_exists( __CLASS__, $update_script ) ) {
				self::$update_script();
			}
		}
	}
	...
	/**
	 * Update older versions which did not have a version flag.
	 *
	 * @since 2.0.0
	 */
	private static function update_to_1_13_5() {
	...
	/**
	 * Upgrade plugin to version 2.0.0.
	 *
	 * @since 2.0.0
	 */
	private static function update_to_2_0_0() {
			...
			if ( isset( $legacy_options['cost'] ) || isset( $legacy_options['discount'] ) ) {
				...
				if ( isset( $price_adjustment_key ) ) {
					...
					if ( ! empty( $price_adjustment ) || is_numeric( $price_adjustment ) ) {
						...
						$adjustment_type   = SV_WC_Helper::str_starts_with( (string) $price_adjustment, '-' ) ? 'discount'   : 'cost';
						$adjustment_unit   = SV_WC_Helper::str_ends_with(   (string) $price_adjustment, '%' ) ? 'percentage' : 'fixed';
		...
		if ( ! empty( $legacy_pickup_locations ) && is_array( $legacy_pickup_locations ) ) {
			...
			require_once( wc_local_pickup_plus()->get_plugin_path() . '/includes/class-wc-local-pickup-plus-post-types.php' );
			...
			foreach ( $legacy_pickup_locations as $legacy_pickup_location ) {
				...
				if ( is_numeric( $post_id ) ) {
					...
					$pickup_location->set_address( array(
					...
					if ( ! empty( $legacy_pickup_location['cost'] ) ) {
						...
						if ( ! empty( $legacy_adjustment ) || is_numeric( $legacy_adjustment ) ) {
							...
							$adjustment_type   = SV_WC_Helper::str_starts_with( (string) $legacy_adjustment, '-' ) ? 'discount'   : 'cost';
							$adjustment_unit   = SV_WC_Helper::str_ends_with(   (string) $legacy_adjustment, '%' ) ? 'percentage' : 'fixed';
			...
			if ( $processed > 0 || $skipped > 0 ) {
				...
				$plugin->get_admin_notice_handler()->add_admin_notice(
					...
					sprintf( __( 'Pickup locations have been upgraded to a new format! All location data should be the same, but %1$splease double-check them here%2$s.', 'woocoommerce-shipping-local-pickup-plus' ), '<a href="' . admin_url( 'edit.php?post_type=wc_pickup_location' ) . '">', '</a>' ),
		...
		wc_local_pickup_plus()->log( 'Upgrade to version 2.0.0 complete.' );
	}
	 ...
	 * @since 2.3.4
	 */
	private static function update_to_2_3_4() {
		global $wpdb;
		...
		// log upgrade complete
		wc_local_pickup_plus()->log( 'Upgraded to version 2.3.4.' );
	}
