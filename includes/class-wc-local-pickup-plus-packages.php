...
class WC_Local_Pickup_Plus_Packages {
	...
	private function create_package( $items ) {
		...
		return array(
			...
			'destination'     => $this->get_package_destination_address( $billing_address, $shipping_address ),
		);
	}
	...
	private function cart_item_should_be_picked_up( $cart_item, $cart_item_key, $pickup_data = array(), $shipping_rates = array() ) {
		...
		// sanity check if the cart item must be picked up by product setting
			...
			// - there are no other shipping options available (shipping rates)
			// - the default package handling is pickup if per order checkout mode is enabled
			// - the default item grouping is not automatic if per order checkout mode is enabled
			// - pickup for the item is possible (item can be picked up)
			if (    ! $pickup
			     &&   empty( $shipping_rates )
			     &&   ( ! $local_pickup_plus->is_per_order_selection_enabled() || $local_pickup_plus->is_default_handling( 'pickup' ) ) ) {
	 ...
	 *
	 * TODO remove this deprecated method by November 2019 or version 3.0.0, whichever comes first {FN 2018-11-22}
	 *
	 * @since 2.3.7
	...
	public function cart_item_may_have_shipping( $cart_item_id ) {
		...
		_deprecated_function( 'WC_Local_Pickup_Plus_Packages::cart_item_may_have_shipping()', '2.3.17' );
